...
from src.workers import poster
...
def login() -> None:
    ...
    ensure_dirs()
    db.init_db()
    from src.plugins.ig import IGClient
    cl = IGClient()
    try:
        cl.login()
    except Exception as e:
...
def add_content(
    ...
    for p in media:
        if not p.exists():
            ...
            raise typer.Exit(1)
    from src.content import dedup, hashtags as hashtags_mod
    cfg = _require_niche()
...
def warmup_status() -> None:
    ...
    db.init_db()
    cfg = _require_niche()
    from src.core.warmup import current_day, effective_caps
...
def reddit_questions_cmd(
    ...
    db.init_db()
    cfg = _require_niche()
    from src.brain import reddit_harvester
...
def controlnet_set(
    ...
    ensure_dirs()
    cfg = _require_niche()
    from src.plugins import controlnet as cn_mod
    try:
