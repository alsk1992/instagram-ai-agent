...
from src.content.style import apply_lut_video, apply_watermark
...
def render_captions(
    ...
    *,
    ...
    from src.content.transcribe import transcribe_words
...
def transcribe_to_srt(audio: Path, srt_out: Path) -> Path:
    from src.content.captions_render import render_srt
    from src.content.transcribe import transcribe_words
...
async def generate(
    ...
    *,
    ...
    with tempfile.TemporaryDirectory(prefix="reel_") as _wd:
        ...
        from src.plugins import audio_mix, beat_sync, music as music_plugin
